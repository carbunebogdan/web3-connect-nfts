...
export function NftCard({ nftData, viewMetaData }) {
    return (
        <article className='flex flex-col rounded-lg bg-white overflow-hidden max-w-[280px] relative'>
            <Image src={nftData.metadata?.image.replace('ipfs://', 'https://ipfs.io/ipfs/')} alt='NFT image' height={280} width={280}></Image>
            <div className='p-5 overflow-hidden'>
                <h2 className='font-semibold text-xl truncate mb-5'>{`MegaKong #${nftData.tokenId}`}</h2>
